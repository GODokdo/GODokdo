...
# 4가지 Status
# - regiested: URL 링크만 등록된 상태(크롤링에 의해 URL이 등록되었을 수도 있고, 사용자가 웹페이지 분석을 요청한 경우일 수도 있음 ) -> 본문을 스크랩해야함
...
# 새로운 문서 등록
# api.addDocument(url="https://naver.com/1", title="타이틀", contents="본문") # 제목과 내용이 있으니 regiested 상태를 건너뜀
...
result = api.addDocument(url=temp_url) # 제목과 내용이 없으니 regiested 상태가 됨.
if result.status_code == 201:
  ...
  # regiested 상태의 문서만 가져오기
  document_list = api.getDocument(status='regiested')
...
document_no = 320
result = api.updateDocument(document_no, status="regiested")
if result.status_code == 201:
...
result = api.addDocument(url=temp_url2) # 제목과 내용이 없으니 regiested 상태가 됨.
if result.status_code == 201:
  print("새로운 URL 등록 성공")
  print("    문서 고유 URL", result.json()["created_document_url"])
